...
public class PlayerController2 : MonoBehaviour {
    ...
    private PlayerMovement2 m_PlayerMovement;
    ...
    public AudioClip m_OnHitFemaleClip;
    public AudioClip m_DeathFemaleClip;
    ...
    void Awake() {
        ...
        m_PlayerMovement = GetComponent<PlayerMovement2>();
	...
	void Update () {
        ...
        if (Input.GetButton("Fire2") && m_CanFire && m_CurrentAmmoType != m_RapidFireAmmoType && !Player2Dead)
        {
            m_CanFire = false;
            StartCoroutine(ShotDelay());
            Instantiate(m_CurrentAmmoType, transform.position, Quaternion.identity, transform);
        }
        else if (Input.GetButton("Fire2") && m_CanFire && m_CurrentAmmoType == m_RapidFireAmmoType && !Player2Dead)
        {
            m_CanFire = false;
            StartCoroutine(RapidFireShotDelay());
            Instantiate(m_CurrentAmmoType, transform.position, Quaternion.identity);
        }
        else if (Input.GetButtonDown("Share2") && HealthP2 > 1 && PlayerController1.HealthP1 < 5)
        {
            HealthP2--;
            PlayerController1.HealthP1++;
        }

        if (HealthP2 > 0 && Player2Dead)
        {
            Player2Dead = false;
        }
        m_IsColliding = false;
    ...
    private void TakeDamage()
    {
        if (HealthP2 > 0)
        {
            m_Source.PlayOneShot(m_OnHitFemaleClip, 0.8f);
            HealthP2--;
        }
        ...
        {
            Player2Dead = true;
            m_Source.PlayOneShot(m_DeathFemaleClip, 1f);
        }
